...
try:
    ...
    print(
        "[main] WARNING: data/datasets.py not found. Use --skip-datasets if already generated."
    )
...
try:
    ...
    def run_clustering(sim_path, output_path, sc, epsilon, verbose=False):
        LPSS_main.cluster_results(sim_path, epsilon, PROJECT_ROOT, sc)
        return {}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="PSCAN full pipeline: dataset → similarity → clustering."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def step_datasets(args: argparse.Namespace) -> None:
    ...
    if not DATASETS_AVAILABLE:
        raise RuntimeError(
            "data/datasets.py is not importable. Check your folder structure."
        )
    ...
    argv = [
        ...
        str(args.max_degree),
        "--min-community",
        str(args.min_community),
        "--max-community",
        str(args.max_community),
        "--ba-m",
        str(args.ba_m),
    ]
...
def step_similarity(args: argparse.Namespace) -> dict:
    ...
    for adjlist_path in adjlist_files:
        ...
        if sim_path.exists() and not args.skip_similarity:
            if args.verbose:
                print(
                    f"[main]   Skipping {adjlist_path.name} (sim file already exists)"
                )
        ...
        print(
            f"[main]   ✓ {adjlist_path.stem}: {len(sim):,} similarities in {elapsed:.2f}s → {sim_path.name}"
        )
        all_sim[adjlist_path.stem] = sim_path
...
def step_clustering(args: argparse.Namespace, sim_files: dict, sc) -> dict:
    ...
    for name, sim_path in sim_files.items():
        ...
        labels = run_clustering(
            sim_path, out_path, sc, epsilon=args.epsilon, verbose=args.verbose
        )
...
def main() -> None:
    ...
    if not args.skip_similarity:
        ...
        adjlist_dir = args.output_dir / "adjlists"
        sim_files = (
            {p.stem: p for p in adjlist_dir.glob("*.sim.tsv")}
            if adjlist_dir.exists()
            else {}
        )
    # Step 3
    if not args.skip_clustering:
        sc = SparkContext("local", "LPCC")
        sc.addPyFile(f"{PROJECT_ROOT}/clustering/LPSS_pyspark.py")
        cluster_results = step_clustering(args, sim_files, sc)
    else:
